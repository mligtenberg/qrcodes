          ...
          <div>
            ...
            {!logoImg?(
              ...
              <div style={{display:'flex',flexDirection:'column',gap:12}}>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:8}}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                    <span style={{fontSize:11,color:'var(--txtM)',fontWeight:500}}>Logo size</span>
                    <div style={{background:'var(--bg)',border:`1.5px solid var(--brd)`,borderRadius:6,padding:'3px 8px',minWidth:42,textAlign:'center',display:'flex',alignItems:'center',justifyContent:'center',gap:1}}><input type="number" min={5} max={50} step={0.5} value={parseFloat((logoR*100).toFixed(1))} onChange={e=>{const v=+e.target.value;if(v>=5&&v<=50)setLogoR(v/100);}} style={{width:36,background:'transparent',border:'none',color:'var(--txt)',fontSize:11.5,fontFamily:'monospace',textAlign:'center',outline:'none'}}/><span style={{fontSize:11.5,fontFamily:'monospace',color:'var(--txt)'}}>%</span></div>
                  </div>
                  <input type="range" min={0.05} max={0.50} step={0.01} value={logoR} onChange={e=>setLogoR(+e.target.value)}/>
                  <div style={{fontSize:10.5,color:'var(--txtD)',marginTop:-2}}>Keep under 30% for reliable scanning</div>
                </div>
