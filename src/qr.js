...
export function generateMatrix(text,ecLevel='L'){
  const qr=qrcodeGenerator(0,ecLevel);qr.addData(text,'Byte');qr.make();
...
export function getVersionForEC(text,ecLevel){
  try{const qr=qrcodeGenerator(0,ecLevel);qr.addData(text,'Byte');qr.make();return Math.floor((qr.getModuleCount()-17)/4);} 
  catch{return 99;}
}
